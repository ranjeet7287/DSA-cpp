...
class Deque 
{
    ...
    void insertFront(int element)
    {
        ...
        if(size==0){
            ...
            first_index=(((first_index - 1) % capacity + capacity) % capacity);
        }
        data[first_index]=element;
        size++;
    }
    // Choice 2
    ...
    void deleteRear(){
        ...
        next_index=((next_index-1)%capacity+capacity)%capacity;
        size--;
        if(size==0){
            first_index=-1;
            next_index=0;
    ...
    int getRear()
    {
        ...
        return data[(((next_index - 1) % capacity + capacity) % capacity)];
    }
};
