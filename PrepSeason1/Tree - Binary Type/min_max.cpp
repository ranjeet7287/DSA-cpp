...
// For a given a Binary Tree of type integer, find and return the minimum and the
// maximum data values.
...
{
    pair<int,int> ans;
    ans.first= INT_MAX; // --> for min val
    ans.second=INT_MIN; // --> for max val
    if(root==NULL){
        return ans;
    }
    pair<int,int> leftPair= getMinAndMax(root->left);
    pair<int,int> rightPair= getMinAndMax(root->right);
    ans.first=min(root->data,min(leftPair.first,rightPair.first));
    ans.second=max(root->data,max(leftPair.second,rightPair.second));
    return ans;
}
