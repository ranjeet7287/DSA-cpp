...
// Given a tree and an integer x, find and return the number of nodes which 
// contains data greater than x.
// Input format:
...
int getlargerthanX(TreeNode<int> *root,int x){
    if(root==NULL){
        return -1;
    }
    int count=0;
    ...
    for(int i=0;i<root->children.size();i++){
        count+=getlargerthanX(root->children[i],x);
    }
    return count;
}
