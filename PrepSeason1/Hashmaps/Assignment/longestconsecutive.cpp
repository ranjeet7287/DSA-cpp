...
// Output format:
// The first and only line of output contains starting and ending element of the longest consecutive 
// sequence. If the length of  the longest consecutive sequence is 1, then just print the starting element.
// Constraints :
// 0 <= n <= 10^6
// Time Limit: 1 sec
// Sample Input 1 :
...
// Sample Output 2 :
// 7 9
// Explanation: Sequence should be of consecutive numbers. Here we have 2 sequences with same 
// length i.e. [1, 2, 3] and [7, 8, 9], but we should select [7, 8, 9] because the starting point
// of [7, 8, 9] comes first in input array and therefore, the output will be 7 9, as we have to print 
...
vector<int> longestConsecutive(int *arr,int n)
{
    ...
    int start,end,maxlenght=0;

    for(int i=0;i<n;i++){
        // intialing with true to value for all keys
        map[arr[i]]=true;
